...
<body>
    ...
    <div class="main w-96 p-10 rounded-md flex items-center flex-col bg-white m-auto my-5 border border-green-800">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label class="block">
                    ...
                    <input type="text" name="lastname"
                        class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800"
                ...
                <label class="block col-span-2">
                    ...
                    <input type="date" name="burial_date"
                        class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800">
                </label>

                <label class="block col-span-2">
                    <span class="text-gray-700">Lot Type:</span>
                    <input type="text" name="lot_type_1" value="<?php echo isset($_GET['lot_type']) ? $_GET['lot_type'] : '' ?>"
                        class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800">
                </label>

                <label class="block col-span-2">
                    <span class="text-gray-700">Area Size (Sq m):</span>
                    <input type="text" id="area_size_1" value="<?php echo isset($_GET['lot_area']) ? $_GET['lot_area'] : '' ?>" name="area_size_1"
                        class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800  ">
                </label>
                ...
                <label class="block col-span-2">
                    ...
                    <input type="text" id="amount_1" name="amount_1" value="<?php echo isset($_GET['amount']) ? $_GET['amount'] : '' ?>"
                        class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800">
                </label>
        ...
        <div class="flex justify-center gap-10 mt-10">
            <button onclick="document.getElementById('my_modal5').showModal()"
                class="rounded-md bg-green-800 text-white w-28 h-12 text-sm flex justify-center items-center">Down
                Payment</button>
            <button type="button" id="pay-now-button"
                class="rounded-md bg-green-800 text-white w-28 h-12 text-sm flex justify-center items-center">Full
                Payment</button>
        </div>

    </div>

    <div id="payment-modal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-8 rounded-lg shadow-lg w-96">
            <h2 class="text-2xl font-bold mb-6 text-center">Lot Payment</h2>
            <div id="payment-form" class="space-y-4">
                <div class="form-row">
                    <label for="card-element" class="block text-sm font-medium text-gray-700 mb-2">Credit or debit
                        card</label>
                ...
                <button id="submit-button"
                    class="w-full bg-green-800 text-white p-3 rounded-md hover:bg-green-700">Pay</button>
            </div>
        </div>
    </div>

    <dialog id="my_modal5" class="modal z-40">
        <div class="modal-box">
            <label class="block col-span-2">
                <span class="text-gray-700">Down Payment:</span>
                <input type="text" id="down_payment_1" name="down_payment_1"
                    class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800">
            </label>

            <label class="block col-span-2">
                <span class="text-gray-700">Monthly Amortization 0% Interest:</span>
                <select id="monthly_amortization_1" name="monthly_amortization_1"
                    class="mt-1 block w-full rounded-md gap-2 p-2 mt-2 w-full text-sm border border-green-800">
                    <?php
                ...
                <div class="flex justify-center gap-10 mt-10">
                    <a href="index.php"
                        class="rounded-md bg-green-800 text-white w-24 p-3 text-sm flex justify-center items-center">Cancel</a>
                    <button type="submit" id="pay-now"
                        class="rounded-md bg-green-800 text-white w-24 p-3 text-sm flex justify-center items-center">Pay
                        now</button>
                </div>
            </div>
        </div>
    ...
    form.addEventListener('submit', function (event) {
        ...
        stripe.createToken(card).then(function (result) {
            if (result.error) {
                ...
                stripeTokenHandler(result.token); // Handle the token
            }
        });
    });

    // Function to handle the token after successful creation
    ...
    var payNowButton2 = document.getElementById('pay-now');
    var downPaymentModal = document.getElementById('my_modal5');

    payNowButton.addEventListener('click', function (event) {
        event.preventDefault();
        downPaymentModal.close(); // Close the down payment modal if open
        paymentModal.classList.remove('hidden');
    });

    payNowButton2.addEventListener('click', function (event) {
        event.preventDefault();
        downPaymentModal.close(); // Close the down payment modal if open
        paymentModal.classList.remove('hidden');
    });
